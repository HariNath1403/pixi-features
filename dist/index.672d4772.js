...
const loadPage = function(target) {
    ...
    if (targetPage) targetPage.classList.remove("hide--display"); // Show the target page
    else console.error(`Element with ID ${target} does not exist.`);
};
// Hide all pages first, then show the selected page
featuresBtn.forEach((btn)=>btn.addEventListener("click", (e)=>{
        e.preventDefault();
...
goToPromotionBtn.forEach((btn)=>btn.addEventListener("click", (e)=>{
        e.preventDefault();
        allPages.forEach((page)=>page.classList.add("hide--display")); // Hide all pages
